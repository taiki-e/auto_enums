use proc_macro2::TokenStream;
use quote::quote;

use crate::utils::*;

pub(crate) const NAME: &[&str] = &["RangeBounds"];

pub(crate) fn enum_derive(data: &syn::ItemEnum) -> Result<TokenStream> {
    EnumData::parse(data, true, true).map(|data| range_bounds(&data, &std_root()))
}

fn range_bounds(data: &EnumData<'_>, root: &TokenStream) -> TokenStream {
    let EnumData {
        name,
        impl_generics,
        ty_generics,
        where_clause,
        variants,
        fields,
    } = data;

    let ops = quote!(#root::ops);
    let trait_ = quote!(#ops::RangeBounds);
    let impl_generics = quote!(#impl_generics __T: ?Sized>);

    let where_clause = fields.iter().fold(where_clause.clone(), |t, f| {
        t.extend_and_return(quote!(#f: #trait_<__T>,))
    });

    quote! {
        impl #impl_generics #trait_<__T> for #name #ty_generics #where_clause {
            #[inline]
            fn start_bound(&self) -> #ops::Bound<&__T> {
                match self { #(#variants(x) => x.start_bound(),)* }
            }
            #[inline]
            fn end_bound(&self) -> #ops::Bound<&__T> {
                match self { #(#variants(x) => x.end_bound(),)* }
            }
        }
    }
}
